...
Manuel played a key role in improving the `Solid` widget and simplifying the providers through numerous PRs. One day, he proposed a radical idea: introducing a `ProviderContext` class that would act like `Context`s in SolidJS or in React. Essentially, `ProviderContext` would serve as the identifier for a scoped `Provider` object, making it safer than relying solely on the type of the provided value (e.g., `context.get<Model>()`).  
